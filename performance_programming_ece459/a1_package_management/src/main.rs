...
fn process_command(state: &mut Packages, cmdline: &str) -> bool {
    ...
    match cmd {
        ...
        // Custom testing command for loading 
        "load-defaults-test" | "ldt" => {
            state.parse_packages("data/test-available-packages");
            state.parse_installed("data/test-installed-packages")
        }
